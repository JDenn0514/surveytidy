...
This document covers conventions that apply to **all surveyverse R packages**. For package-specific examples and details, see the local conventions file in each package (e.g., `surveytidy-conventions.md`).
...
See `surveytidy-conventions.md` for exact family definitions.
...
#' Resolve tidy-select expression to column names
#'
#' @param expr A quosure or character vector.
#' @param data A data.frame.
...
- S3 method implementations (e.g., `filter.survey_base`) — registered in `.onLoad()`, not via NAMESPACE export
...
```r
# Before committing changes to R/filter.R
devtools::document()
git add NAMESPACE man/filter.survey_base.Rd
git commit -m "docs(filter): update roxygen"
```
...
### Package documentation (surveytidy-package.R)
...
#' from surveycore, allowing domain-aware filtering, column selection, variable
#' mutation, and grouping while maintaining proper variance estimation.
#'
#' @section Key Functions:
#' - [filter()] — domain-aware filtering
#' - [select()] — column selection
#' - [mutate()] — add/modify variables
#' - [rename()] — rename variables (auto-updates metadata)
#' - [arrange()] — sort rows
#' - [group_by()] / [ungroup()] — set/clear grouping
#' - [drop_na()] — domain-aware NA filtering
#'
#' @keywords internal
"_PACKAGE"
```
...
**For package-specific details**, see `surveytidy-conventions.md`.

...
## Step 1b: Snapshot failures — route to agent first

If the CI failure type is **snapshot mismatch** (error log contains
"snapshot not equal" or "snapshot has changed"):

→ Launch the `snapshot-reviewer` agent. Paste the full CI diff output into
  the agent prompt. Act on its APPROVE / REJECT / ⚠️ REVIEW classification
  for each snapshot before touching any `_snaps/` files.

Only proceed to Step 2 after the agent has classified all changed snapshots.

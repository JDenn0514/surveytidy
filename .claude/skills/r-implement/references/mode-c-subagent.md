...
- Paths to: `code-style.md`, `testing-surveytidy.md`, `plans/error-messages.md`
- Current branch name
...
> (3) every output column or return type matches the output contract,
> (4) domain column behavior matches the spec (for filter-related verbs),
...
> "Review the code changes against surveytidy conventions:
> (1) S3 methods registered correctly via `registerS3method()` in `.onLoad()`?
>     No `UseMethod()` on survey objects?
> (2) `class=` on every `cli_abort()` and `cli_warn()` call?
> (3) No `@importFrom` anywhere; all external calls use `::`?
> (4) `test_invariants()` first assertion in every verb test block?
> (5) Dual pattern (class= + snapshot) on all user-facing error tests?
> (6) Domain column preserved through operation (tested)?
> (7) All 3 design types tested via `make_all_designs()`?
> (8) All examples begin with `library(dplyr)` or `library(tidyr)`?

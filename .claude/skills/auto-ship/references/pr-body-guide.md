...
**Good:**
> This PR adds domain-aware filtering for all three survey design types. Before
> this, you had to physically remove rows with `subset()`, which issues a
> warning and breaks variance estimation. Now `filter()` marks rows in or out
> of the analysis domain without removing them, so design-based variance
> estimators stay valid.

**Too vague:**
> This PR implements Phase 0.5 filter functionality.

**Too technical:**
> Implements domain column accumulation via AND-composition of quosure
> conditions stored in @variables$domain.
...
- **New error classes:** `surveytidy_error_X`, `surveytidy_warning_Y`
...
- [x] behavior 1
- [x] behavior 2
- [x] error condition A — `surveytidy_error_X`
```

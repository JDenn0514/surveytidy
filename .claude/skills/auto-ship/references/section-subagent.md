...
You are the implementation agent for **one section** of a surveytidy
...
- [ ] S3 methods registered correctly via `registerS3method()` in `.onLoad()`?
...
- [ ] `test_invariants(design)` is the first assertion in every verb test block
...
Scopes: `filter`, `select`, `mutate`, `rename`, `arrange`, `group-by`, `tidyr`,
`joins`, `result`, `utils`, `zzz`.
